...
def create_app():  # noqa: C901
    ...
    async def _enque_file(
        ...
        for i, file in enumerate(files):
            ...
            # Detect MIME type
            mime_type, detection_method = mime_detector.detect(file.filename, content)
            ...
            if file.filename:
                ...
                if mime_type:
                    name = mime_detector.generate_filename(mime_type, i)
                else:
                    # Fallback to generic name
                    suffix = "" if len(file_sources) == 1 else f"_{i}"
                    name = f"file{suffix}.bin"
            
            _log.info(f"Processing file: {name} (MIME: {mime_type}, detected via: {detection_method})")
            
            # Validate if it's a supported format
            if mime_type and mime_detector.is_supported_image(mime_type):
                _log.info(f"Detected supported image format: {mime_type}")
